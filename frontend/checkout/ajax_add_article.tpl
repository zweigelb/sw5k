            ...
            {block name='checkout_ajax_add_information_image'}
                <div class="article--image block">
                    <a href="{$detailLink}" class="link--article-image" title="{$sArticle.articlename|escape}">
                        ...
                        {$alt = $sArticle.additional_details.articleName|escape}
                ...
                {block name='checkout_ajax_add_information_name'}
                    <div class="article--name">
                        <ul class="list--name list--unstyled">
                            <li class="entry--name">
                                <a class="link--name" href="{$detailLink}" title="{$sArticle.additional_details.articleName|escape}">
                                    {$sArticle.additional_details.articleName|escape|truncate:35}
                                </a>
                            </li>
                            <li class="entry--ordernumber">{s name="AjaxAddLabelOrdernumber"}{/s}: {$sArticle.ordernumber}</li>
                            {foreach $sArticle.article.sConfigurator as $group}
                                {if $group.groupname && $group.values[$group.selected_value].optionname}
                                    <li class="entry--configurator">{$group.groupname}: {$group.values[$group.selected_value].optionname}</li>
                                {/if}
                            {/foreach}
                        </ul>
                    </div>
                ...
                {block name='checkout_ajax_add_information_price'}
                    <div class="article--price">
                        <ul class="list--price list--unstyled">
                            <li class="entry--price">
                                {if $sArticle.article.pseudoprice && $sArticle.article.pseudoprice > $sArticle.price}
                                <span class="nxsprice--pseudo">{$sArticle.article.pseudoprice|currency}</span>
                                {/if}
                                <span class="{if $sArticle.article.pseudoprice && $sArticle.article.pseudoprice > $sArticle.price}nxsprice--pseudo--active{/if}">{$sArticle.price|currency} {s name="Star" namespace="frontend/listing/box_article"}{/s}</span>
                            </li>
