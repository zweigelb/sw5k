{block name="frontend_note_item"}
    <div class="note--item panel--tr">
        ...
        {if $sBasketItem.sConfigurator}
            {$detailLink={url controller="detail" sArticle=$sBasketItem.articleID number=$sBasketItem.ordernumber_raw}}
        {else}
            ...
            <div class="note--sale panel--td">
                ...
                {block name="frontend_note_item_price"}
                    {if $sBasketItem.itemInfo}
                        <span class="{if $sBasketItem.pseudoprice}nxsprice--pseudo--active{/if}">{$sBasketItem.itemInfo}</span>
                    {else}
                        <div class="note--price">
                            {* Discount price *}
                            {block name='frontend_listing_box_article_price_discount'}
                                {if $sBasketItem.pseudoprice && $sBasketItem.pseudoprice > $sBasketItem.price}
                                 <span class="nxsprice--pseudo">
                                    {$sBasketItem.pseudoprice|currency|regex_replace:"/,\d+/":""}
                                </span>
                                {/if}
                            {/block}
                            <span class="{if $sBasketItem.pseudoprice && $sBasketItem.pseudoprice > $sBasketItem.price}nxsprice--pseudo--active{/if}">{if $sBasketItem.priceStartingFrom}{s namespace='frontend/listing/box_article' name='ListingBoxArticleStartsAt'}{/s} {/if}{$sBasketItem.price|currency|regex_replace:"/,\d+/":""}*</span>
                        </div>
                    {/if}
                {/block}
                        ...
                        <span class="note--price-unit">
                            ...
                            {if $sBasketItem.purchaseunit != $sBasketItem}
                                {if $sBasketItem.referenceunit}
                                    ({$sBasketItem.referenceprice|currency|regex_replace:"/,\d+/":""} {s name="Star" namespace="frontend/listing/box_article"}{/s} / {$sBasketItem.referenceunit} {$sBasketItem.sUnit.description})
                                {/if}
                            {/if}
                        </span>
                    {/if}
